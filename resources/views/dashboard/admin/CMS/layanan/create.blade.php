...
<div class="card shadow mb-4" style="width: 50rem;">
    ...
    <div class="card card-body">
        <form id="myForm" method="POST" action="{{ route('layanan.store') }}" enctype="multipart/form-data">
            @csrf
        ...
        <div class="mb-3">
            ...
            <input type="number" name="price" id="price" class="form-control @error('caption') is-invalid @enderror" value="{{ old('price') }}">
            @error('price')
                <div class="text-danger"><small>{{ $message }}</small></div>
            @enderror
        </div>
